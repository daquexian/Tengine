            ...
            for(int ph = 0; ph < param->outh; ph++)
            {
                for(int pw = 0; pw < param->outw; pw++)
                {
                    ...
                    for(int h = h_start; h < h_end; h++)
                    {
                        ...
                        {
                            val = fp16_to_fp32(data[i*param->inh*param->inc * param->inw +h * param->inc * param->inw + w * param->inc +c]);
                            tmp += val * val;
                            pool_size++;
                        }
                    }
                    if(tmp == 0)
                    {
                        out_data[index] = fp32_to_fp16(0.f);
                    }
                    else
                    {
                        out_data[index] = fp32_to_fp16(sqrt(tmp / pool_size));    
                    }
                }
            }
        }
            ...
            for(int ph = 0; ph < param->outh; ph++)
            {
                for(int pw = 0; pw < param->outw; pw++)
                {
                    ...
                    for(int h = h_start; h < h_end; h++)
                    {
                        for(int w = w_start; w < w_end; w++)
                        {
                            val = data[i*param->inh*param->inc * param->inw +h * param->inc * param->inw + w * param->inc +c];
                            tmp += val * val;
                            pool_size++;
                        }
                    }
