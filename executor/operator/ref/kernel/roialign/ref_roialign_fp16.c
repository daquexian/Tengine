...
static inline float bilinear_interpolate_fp16(fffffp16* ptr, int w, int h, float x, float y)
{
    ...
    float r0 = fp16_to_fp32(ptr[ y0 * w + x0 ]) * a0 + fp16_to_fp32(ptr[ y0 * w + x1 ]) * a1;
    float r1 = fp16_to_fp32(ptr[ y1 * w + x0 ]) * a0 + fp16_to_fp32(ptr[ y1 * w + x1 ]) * a1;
            ...
            for(int pw = 0; pw < w; pw++){
                ...
                for (int by = 0; by < bin_grid_h; by++)
                {
                    ...
                    for (int bx = 0; bx < bin_grid_w; bx++)
                    {
                        ...
                        float v = bilinear_interpolate_fp16(ptr, w, h, x, y);

                        sum += v;
                    }
                }      
                outptr[pw] = fp32_to_fp16(is_empty ? 0.f : (sum / (float)area));        
            }
            outptr += w;
        }
